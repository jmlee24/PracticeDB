"""day3/complete/routes/categories.py — Day1/2 와 동일."""
from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session

from day3.complete.database import get_db
from day3.complete.models import Category
from day3.complete.schemas import CategoryCreate, CategoryResponse
...
def update_category(category_id: int, data: CategoryCreate, db: Session = Depends(get_db)):
    ...
    if not category:
        raise HTTPException(status_code=404, detail="카테고리를 찾을 수 없습니다")
    category.name = data.name; category.description = data.description; category.is_active = data.is_active
