"""day3/practice/routes/brands.py — Brand CRUD (완성, 참고용)"""
from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session

from day3.practice.database import get_db
from day3.practice.models import Brand
from day3.practice.schemas import BrandCreate, BrandResponse
...
def list_brands(is_active: bool | None = Query(default=None), db: Session = Depends(get_db)):
...
def update_brand(brand_id: int, data: BrandCreate, db: Session = Depends(get_db)):
    ...
    if not brand:
        raise HTTPException(status_code=404, detail="브랜드를 찾을 수 없습니다")
    brand.name = data.name; brand.description = data.description; brand.is_active = data.is_active
